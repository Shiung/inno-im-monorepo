<script lang="ts">
  ...
  const getStamp = (hitStatus: IArticle['hitStatus']) => {
    switch (hitStatus) {
      case 1:
        return ['zh_CN', 'zh_HK'].includes($locale) ? cnWinStamp : enWinStamp
      case 2:
      default:
        return ''
    }
  ...
  $: stampStyle = hideStamp
    ? null
    : `background-image: url(${getStamp(
        article.hitStatus
      )});background-position: right -13px top -15px; background-size: 72px`
</script>

<Ripple
  class={twMerge('w-full rounded-[10px] im-shadow bg-no-repeat p-[8px]', $$props.class)}
  style={stampStyle}
  on:click={onClick}
>
