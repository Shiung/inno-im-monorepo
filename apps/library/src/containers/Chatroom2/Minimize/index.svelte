...
const calculateUnread = (msgs: IChatMessage[], _lastReadId: string) => {
  if (!_lastReadId) return msgs.length

  const lastIdx = msgs.findIndex(msg => msg.id === _lastReadId)
...
<div class='fixed bottom-[31px] w-[100vw] px-[20px]'> 
  <Ripple class='flex items-center w-full im-shadow rounded-[20px] h-[48px] px-[20px]' 
  ...
  >
    <div class='relative'>
      ...
      {#if unread !== 0}
        <div class='absolute bg-imprimary rounded-full top-[-30%] right-[-40%] px-[2px] text-[10px] text-white font-semibold min-w-[15px]'>
          {unread}
        </div>
      {/if }
